...
contract Swap {
    ...
    function swapEthForLink (uint256 _amount) external {
        ...
        require(wethTokenAddr.balanceOf(msg.sender) >= _amount, "You dont not have enough tokens to swap");
    ...
    function swapLinkForEth (uint256 _amount) external {
        ...
        // confirming user balance
        require(linkTokenAddr.balanceOf(msg.sender) >= _amount, "You dont not have enough tokens to swap");

        // transfer ether from user to contract
        require(linkTokenAddr.transferFrom(msg.sender, address(this), _amount), "transaction not succesful");
    ...
    function swapDaiForEth (uint256 _amount) external {
        ...
        // confirming user balance
        require(daiTokenAddr.balanceOf(msg.sender) >= _amount, "You dont not have enough tokens to swap");

        // transfer ether from user to contract
        require(daiTokenAddr.transferFrom(msg.sender, address(this), _amount), "transaction not succesful");
    ...
    function swapEthForDai (uint256 _amount) external {
        ...
        // transfer ether from user to contract
        require(wethTokenAddr.transferFrom(msg.sender, address(this), _amount), "transaction not succesful");
